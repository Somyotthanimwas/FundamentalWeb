...
try {
    ...
    // Check the V5 Fundamental CSV every 1 second.
    // When FundamentalUpdater_rev5 updates the CSV,
    // reload the data immediately without waiting for an API request.
    // ============================================================
    std::thread csvWatcher(
        [&]() {
            while (true) {
